...
def descobre_atividades(file): # recebe o nome do arquivo a ser tratado
    ...
    for row in f: # itera as linhas do arquivo
        ...
        if line[2] in atividades: # verifica se o nome da atividade já existe no vetor
            continue # se sim, passa para a próxima iteração
        else:
            atividades.append(line[2]) # se não, adiciona o nome da atividade no vetor
            i = i + 1
