...
class cMLP(object):
    ...
    def treinar_MLP(self,Xtr, Ytr,XVal, YVal,nitmax, alfa): # TODO add accuracy
        ...
        self.WA=np.random.randn(self.no,ne+1)/10 # mtrix pesos no x ne
        ...
        if(not(np.all(XVal==0))):
            YVal_out=self.calc_saida(XVal,self.WA,self.WB,N_val,self.funcao_f,self.funcao_g)
        ...
        while(EQM>=1e-6 and nit<nitmax and nit_val<10000):
            ...
            vet_erro=np.concatenate((vet_erro,EQM),axis=1)

            # validacao
            if(not(np.all(XVal==0))):
                YVal_out=self.calc_saida(XVal,self.WA,self.WB,N_val,self.funcao_f,self.funcao_g)
                erro_val=YVal_out-YVal
                EQM_val=sum(sum(erro_val.dot(erro_val)))/N_val
                vet_erro_val=np.concatenate((vet_erro_val, EQM_val),axis=1)
    ...
    def calc_saida(self,X,WA,WB,N,funcao_f,funcao_g):
        Zin=X@(WA.conj().T) # obs: pode ser *?
        Z=self.ativacao(funcao_f,Zin)
        #Yin=[ones(N,1),Z]*WB';
        Yin=np.concatenate((np.ones((N,1),float),Z),axis=1)@(WB.conj().T)
    ...
    def calc_grad(self,Xtr,Z,Y,erro,WB,N,funcao_f,funcao_g):
        ...
        grad_WB = 1/N@((erro*dg).conj().T)@(np.concatenate((np.ones((N,1),float), Z),axis=1))
        #dJdZ = (erro.*(dg))*WB(:,2:end);
        dJdZ = (erro*dg)@WB[1:]
        #grad_WA = 1/N*(dJdZ.*(df))'*Xtr;
        grad_WA = 1/N@((dJdZ*df).conj().T)@Xtr
